...
def test_batch_mode():
    ...
    for (image, item) in images:
        # image_info, item_info = images
        input = f'{{"Text_sentence": "{item["context"]}", "Image_URL": "{image["image_url"]["url"]}"}}'
        messages = {
    ...
    with open('result/zero-shot-base/samples1500/gpt-4o-mini/gmner-no-vg-base/batch_request.json', 'w', encoding='utf-8') as f:
    ...
    # 加载jsonl文件
    # responses = []
    # with open('result/zero-shot-base/gmner-no-vg-base/batch_67ee2b1a2d18819080d52f7ded6cdf85_output.jsonl', 'r', encoding='utf-8') as f:
    #     for line in f:
    #         if line.strip():  # 忽略空行
    ...
    for response in responses:
        ...
        if len(json_starts) > 0:
            ...
            # raise ValueError("substring '```json' not found in content")
    ...
    with open('result/zero-shot-base/samples1500/gpt-4o-mini/gmner-no-vg-base/gmner_no_vg_results.json', 'w', encoding='utf-8') as f:
