...
def mrc2prompt(mrc_data_path, picture_folder_path):
    ...
    for item_idx in tqdm(range(len(mrc_data))):
        ...
        matched_image_id = item_["matched_image_ids"]
        results.append(({
                            "type": "image_url",
                            "image_url": {
                                "url": image2prompt(picture_folder_path, image_id)
                            }
                        }, item_, matched_image_id))
    return results
...
def test_batch_mode():
    ...
    # 读取 all_prompts.md
    prompts_file_path = r'prompts/zero-shot-demo/NER/NER_system_cot.md'
    prompts = load_prompts(prompts_file_path)
    ...
    # 加载一个json文件
    with open(r'result/zero-shot-demo/ner-demo/ner_demo_correct.json', 'r', encoding='utf-8') as f:
        demo = json.load(f)
    ...
    for (image, item, matched_image_id) in images_text_matched_image_id:
        matched_image_id = matched_image_id[:1]
        for cot in demo:
            if cot.get('image_id') == matched_image_id:
                # 找到匹配项，返回entities字段
                entities = cot.get('entities')
                current_prompt = prompts
                entities_json = json.dumps(entities, ensure_ascii=False, separators=(',', ':'))  # 移除缩进，生成紧凑的单行JSON               
                current_prompt = current_prompt.replace('{{EXAMPLES_PLACEHOLDER}}', entities_json)
                messages = {
